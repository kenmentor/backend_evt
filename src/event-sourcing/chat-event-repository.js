/**
 * Chat Event Repository - MongoDB Version
...
const conversationEventHandlers = {
  ...
  participantAdded: async (id, evt, repo) => {
  ...
  messageRead: async (id, evt, repo) => {
    const existing = await repo.findById(id);
    if (existing) {
      await repo._updateInReadModel(id, { unreadCount: { ...existing.unreadCount, [evt.userId]: 0 } });
    }
  },
  propertyContextUpdated: async (id, evt, repo) => repo._updateInReadModel(id, { propertyContext: evt.propertyContext }),
};

const conversationCommands = {
  create: async (cmd, agg) => {
    if (agg.version > 0) throw new Error('Conversation already exists');
    return { type: 'created', participants: cmd.participants, participantNames: cmd.participantNames, participantAvatars: cmd.participantAvatars, propertyContext: cmd.propertyContext };
  },
  addParticipant: async (cmd, agg) => {
    ...
    return { type: 'participantAdded', participantId: cmd.participantId, participantName: cmd.participantName };
  },
  sendMessage: async (cmd, agg) => {
    if (agg.version === 0) throw new Error('Conversation not found');
    if (!agg.participants.includes(cmd.senderId)) throw new Error('Sender not in conversation');
    return { type: 'messageSent', content: cmd.content, senderId: cmd.senderId };
  },
  markAsRead: async (cmd, agg) => {
    if (agg.version === 0) throw new Error('Conversation not found');
    return { type: 'messageRead', userId: cmd.userId };
  },
  updatePropertyContext: async (cmd, agg) => {
    if (agg.version === 0) throw new Error('Conversation not found');
    return { type: 'propertyContextUpdated', propertyContext: cmd.propertyContext };
  },
};
...
const messageEventHandlers = {
  sent: async (id, evt, repo) => {
    await repo._addToReadModel(id, {
      conversationId: evt.conversationId, senderId: evt.senderId, receiverId: evt.receiverId, content: evt.content, read: false,
    });
  },
  read: async (id, _, repo) => repo._updateInReadModel(id, { read: true }),
  deleted: async (id, _, repo) => repo._updateInReadModel(id, { content: '[deleted]' }),
};

const messageCommands = {
  send: async (cmd, agg) => {
    if (agg.version > 0) throw new Error('Message already sent');
    return { type: 'sent', conversationId: cmd.conversationId, senderId: cmd.senderId, receiverId: cmd.receiverId, content: cmd.content };
  },
