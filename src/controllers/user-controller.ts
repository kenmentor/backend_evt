...
import { response, generateTokenAndSetCookie } from "../utility";
import { user_service, verification_service } from "../service";
...
async function edit_user_detail(req: Request, res: Response) {
  const id = req.params.id as string;
  const { body, file } = req as any;

  const uploadBufferToCloudinary = (fileBuffer: Buffer, folder = "user") => {
  ...
  try {
    ...
    if (file && file.buffer) {
      const result: any = await uploadBufferToCloudinary(file.buffer, "profileImage");
...
async function upload_profile_picture(req: Request, res: Response) {
  const id = req.params.id as string;
...
async function login_user(req: Request, res: Response) {
  ...
  try {
    ...
    generateTokenAndSetCookie(res, data.userId.toString());
...
async function delete_user(req: Request, res: Response) {
  try {
    ...
    const data = await user_service.delete_user(id);
    return res.json(goodResponse(data, "User deleted successfully"));
  } catch (error: any) {
    return res.status(500).json(badResponse(error.message, 500, error));
  }
}
async function find_users(req: Request, res: Response) {
  ...
  try {
    const data = await user_service.find_users({
      ...
      bardge: parseInt(bardge as string),
      id: id,
...
async function pioneer(req: Request, res: Response) {
  try {
