...
const create_request = async (req, res) => {
    ...
    const body = req.body
    try {
        const already = await requst_service.alreadyExit({ host: body.hostId, guest: body.guestId, house: body.houseId })
...
const update_request = async (req, res) => {
    ...
    try {
        ...
        const data = await requst_service.update_request(requestId,{accepted:status==="1" })
